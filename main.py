...
class TicTacToe:
    def __init__(self, game):
        self.game = game
        self.field_image = self.get_scaled_image(path='files/field.png', res=[WIN_SIZE] * 2)
        self.O_image = self.get_scaled_image(path='files/o.png', res=[CELL_SIZE] * 2)
        self.X_image = self.get_scaled_image(path='files/x.png', res=[CELL_SIZE] * 2)
    ...
    def draw_winner(self):
        if self.winner:
            pg.draw.line(self.game.screen, 'purple', *self.winner_line, CELL_SIZE // 8)
            label = self.font.render(f'Player "{self.winner}" wins!', True, 'black', 'light blue')
